...
void Frame::Updated(LogData& data)
{
    if(_status)
    {
        ...
        else
        {
            ...
            if(data.GetLevelLogCount(LOG_FATAL) > 0)
            {
                str << " - " << data.GetLevelLogCount(LOG_FATAL) << " fatal(s) ";
            }
            if(data.GetLevelLogCount(LOG_CRITICAL) > 0)
            {
                str << " - " << data.GetLevelLogCount(LOG_CRITICAL) << " critical(s) ";
            }
            if(data.GetLevelLogCount(LOG_ERROR) > 0)
            {
                str << " - " << data.GetLevelLogCount(LOG_ERROR) << " error(s) ";
            }
            if(data.GetLevelLogCount(LOG_WARNING) > 0)
            {
                str << " - " << data.GetLevelLogCount(LOG_WARNING) << " warning(s) ";
            }
            if(data.GetLevelLogCount(LOG_INFO) > 0)
            {
                str << " - " << data.GetLevelLogCount(LOG_INFO) << " info(s) ";
            }
            if(data.GetLevelLogCount(LOG_DEBUG) > 0)
            {
                str << " - " << data.GetLevelLogCount(LOG_DEBUG) << " debug(s) ";
            }
            if(data.GetLevelLogCount(LOG_TRACE) > 0)
            {
                str << " - " << data.GetLevelLogCount(LOG_TRACE) << " trace(s) ";
            }
    ...
    if(_begin)
    {
        _begin->SetValue(TimestampToDateTime(data.GetBeginDate()));
        _begin->SetDefault(TimestampToDateTime(data.GetBeginDate()));
    }
    if(_end)
    {
        _end->SetValue(TimestampToDateTime(data.GetEndDate()));
        _end->SetDefault(TimestampToDateTime(data.GetEndDate()));
    }
}
...
void Frame::OnLoggerShowOnlyCurrent(wxCommandEvent& event)
{
    if (_logs->GetSelectedItemsCount() > 0) {
        LogEntry& entry = _logModel->Get(_logs->GetSelection());
...
void Frame::OnLoggerFocusPrevious(wxCommandEvent& event)
{
    if (_logs->GetSelectedItemsCount() > 0) {
        ...
        LogEntry& entry = _logModel->Get(pos);
        int logger = entry.logger;
        while (pos > 0)
        {
            --pos;
            LogEntry& e = _logModel->Get(pos);
            if (e.logger == logger)
            {
...
void Frame::OnSetAsBegin(wxCommandEvent& event)
{
    if(_logs->GetSelectedItemsCount()>0)
    {
        ...
        LogEntry& entry = _logModel->Get(row);
        _begin->SetValue(TimestampToDateTime(entry.date));
...
void Frame::OnSetAsEnd(wxCommandEvent& event)
{
    if(_logs->GetSelectedItemsCount()>0)
    {
        ...
        size_t row = _logModel->GetRow(sel);
        LogEntry& entry = _logModel->Get(row);
        _end->SetValue(TimestampToDateTime(entry.date));
...
{
    LogLevel level = (LogLevel)_criticalitySlider->GetValue();
...
{
    wxGetApp().GetFilteredLogData().SetStartDate(wxDateTimeToTimestamp(event.GetDate()));
}

void Frame::OnEndDateEvent(wxDateEvent& event)
{
    wxGetApp().GetFilteredLogData().SetEndDate(wxDateTimeToTimestamp(event.GetDate()));
}
    ...
    if(_logModel->Count()>0 && !str.IsEmpty()) // No search if no content nor nothing to search.
    {
        if(dirNext) // Search ascent (from top to bottom)
        {
            ...
            while(next!=cur)
            {
                ...
                if(find(std::string(_logModel->Get(next).message)))
                {
